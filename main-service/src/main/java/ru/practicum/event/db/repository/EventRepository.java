...
public interface EventRepository extends JpaRepository<Event, Integer> {
    ...
    Page<Event> getEvents(@Param("users") List<Integer> users, @Param("states") List<String> states,
    ...
    @Query(value = "SELECT requests.id, requests.created," + "       " +
            "requests.event, requests.requester, " + "       events.state " +
    ...
    @Query(value = "select e from Event as e " + "where (lower(e.annotation) like lower(concat('%',:text,'%')) or "
            + "lower(e.description) like lower(concat('%',:text,'%'))) AND "
            + "e.category.id =:categories AND e.paid = :paid AND " + "e.eventDate BETWEEN :rangeStart AND :rangeEnd " +
            "AND e.state= 'PUBLISHED' " + "order by e.eventDate desc")
    List<Event> getEventsWithFilterSortEventDate(@Param("text") String text, @Param("categories") List<Integer> ids,
    ...
    @Query(value = "select e from Event as e " + "where (lower(e.annotation) like lower(concat('%',:text,'%')) or " +
            "lower(e.description) like lower(concat('%',:text,'%'))) AND " + "e.category.id =:categories " +
            "AND e.paid = :paid AND " + "e.state= 'PUBLISHED' AND e.eventDate BETWEEN :rangeStart AND :rangeEnd ")
    List<Event> getEventsWithFilterSortViews(String text, List<Integer> categories, Boolean paid,
                                             LocalDateTime rangeStart, LocalDateTime rangeEnd, Pageable pageable);


    @Query(value = "select e from Event as e " + "where (lower(e.annotation) like lower(concat('%',:text,'%')) or "
            + "lower(e.description) like lower(concat('%',:text,'%'))) AND " + "e.category.id =:categories " +
            "AND e.paid = :paid AND " + "e.state= 'PUBLISHED' AND e.eventDate>=now()")
