...
public interface EventRepository extends JpaRepository<Event, Integer> {
    ...
    @Query(value = "select e from Event e where e.user.id IN :users AND e.state " +
            "IN :states AND e.category.id IN :categories AND e.eventDate " +
    ...
    @Modifying
    @Query(value = "SELECT requests.id, requests.created, requests.event, requests.requester, " +
            " events.state FROM requests JOIN events  on requests.event = events.id  " +
    ...
    @Query(value = "SELECT * FROM events " +
            "WHERE id=?1 AND state like 'PUBLISHED'", nativeQuery = true)
